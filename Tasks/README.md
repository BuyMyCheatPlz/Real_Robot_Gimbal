...
S1 使用相同的三档关系控制 M2006 ID5。修改
`LAUNCH_M2006_ID5_MAX_SPEED_RPM` 后，S1=3 对应新的最大转速，S1=1 保持停止。
M2006 离线时不会释放启动信号量。
...
VOFA 始终以 10 ms 周期发送这组诊断帧；即使 Yaw 被撤权，也可从通道 12～13
判断电机在线与撤权原因。UART4 命令接收保持开启。
