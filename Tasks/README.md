...
`LAUNCH_M2006_ID5_CONTINUOUS_SPEED_RPM`=4800rpm 即 20Hz）；S1=3 单动（角度环，
每次从 1 拨到 3 触发一步 `LAUNCH_M2006_ID5_STEP_DEG`=40° 输出，电机转 4 圈）。
M2006 离线时不会释放启动信号量。
...
JustFloat 通道（前 4 个为目标/实际角 rad→°，后 2 个为 M2006 目标/实际角电机轴°），
帧尾为 `00 00 80 7F`：
