                              ...
                              GPIO_TypeDef *gyro_cs_port, uint16_t gyro_cs_pin)
{
    ...
    if (write_reg(imu, gyro_cs_port, gyro_cs_pin, BMI088_GYRO_INT_CTRL_REG,
                  0x80U) != HAL_OK) return HAL_ERROR;

    imu->initialized = 1U;
    return BMI088_Read(imu);
}
...
{
    if ((imu == 0) || (imu->initialized == 0U)) return HAL_ERROR;
    if ((imu->dma_busy != 0U) || (dma_state != BMI088_DMA_IDLE)) return HAL_BUSY;
    dma_imu = imu;
    imu->dma_busy = 1U;
    imu->dma_data_ready = 0U;
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
    ...
    switch (dma_state)
    {
        case BMI088_DMA_ACCELERATION:
            ...
            (void)start_dma(dma_imu, dma_imu->gyro_cs_port,
                            dma_imu->gyro_cs_pin, BMI088_GYRO_DATA_REG, 7U,
                            BMI088_DMA_GYROSCOPE);
            break;
        ...
        case BMI088_DMA_GYROSCOPE:
            ...
            (void)start_dma(dma_imu, dma_imu->acc_cs_port,
                            dma_imu->acc_cs_pin, BMI088_ACC_TEMP_REG, 4U,
                            BMI088_DMA_TEMPERATURE);
            break;
...
void HAL_SPI_ErrorCallback(SPI_HandleTypeDef *hspi)
{
    ...
    deselect(dma_imu->gyro_cs_port, dma_imu->gyro_cs_pin);
    dma_state = BMI088_DMA_IDLE;
    dma_imu->dma_status = HAL_ERROR;
    dma_imu->dma_busy = 0U;
    dma_imu->dma_data_ready = 0U;
}
