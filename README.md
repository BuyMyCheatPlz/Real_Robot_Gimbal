...
- Pitch 位置环将 GM6020 编码器展开增量换算到 IMU Pitch 坐标作为实际值，速度环使用
  BMI088 陀螺 pitch 角速度作为反馈（直接测云台真实角速度，不受减速/背隙/柔性影响）。
- Pitch 编码器反馈经过圈数展开和一阶低通滤波，可跨越编码器零点连续计算角度。
- 上电/遥控重连后通过首次 BMI088 Pitch 姿态给 GM6020 编码器建立角度零偏。
- Pitch 默认回到 IMU 水平(`PITCH_GRAVITY_ZERO_RAD`=-90°)位置，而不是回电机编码器机械零点。
- 使用 BMI088 Pitch 姿态（带 -90° 零偏，水平=0）计算正弦重力电压前馈，降低云台负载对 PID 的影响。
...
- Pitch/Yaw 均具有目标速度限幅、位置积分限幅、电流输出限幅和角度软限位。
...
| 遥控器上线 | Pitch 回到水平位置，Yaw 回到 BMI Yaw 零点 |
...
| `VOFA_print` | 10 ms | UART4 DMA 发送 9 通道 JustFloat |
...
- 外层位置 PID 输出目标转速；带目标死区（`PITCH_POSITION_DEADZONE_RAD`，默认 0.5°），
  误差小于死区时输出 0 并复位位置 PID，防止齿距背隙在目标附近高频抖动。
- 内层速度 PID 输出 GM6020 电压命令，速度反馈用 BMI088 陀螺 pitch 角速度。
- 根据 BMI088 Pitch（带 -90° 零偏）计算正弦重力电压前馈。
- 上电后用首次有效 BMI Pitch 给编码器建立零偏，目标设为 IMU 水平(-90°)。
- 使用最大转速、积分、输出保护，以及实测机械限位钳位（IMU Pitch -139°~-64°）。
...
D-BUS 通道在代码中使用零基数组下标，`config.h` 已将其对应到实际 CH4～CH9：
...
- Pitch/Yaw 位置/速度 PID、速度限幅、积分限幅、电流输出限幅和角度软限位。
