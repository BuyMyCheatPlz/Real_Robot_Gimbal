...
- Pitch 位置环使用 GM6020 编码器展开角度作为实际值，速度环使用 BMI088 陀螺 pitch
  角速度作为反馈（直接测云台真实角速度，不受减速/背隙/柔性影响）。
...
- Pitch 目标直接钳位到实测机械限位（`PITCH_LIMIT_MIN_RAD`/`PITCH_LIMIT_MAX_RAD`，编码器角度）。
...
- 使用最大转速、积分、输出保护，以及实测机械限位钳位（编码器角度 -125°~-52.37°）。
