...
- M2006 速度环按相位裕度整定：KP=12、LPF alpha=0.7、输出限幅 6500、到位死区
  0.8°（原 KP=24/alpha=0.35 相位裕度只剩约 6°，连发巡航段 I6 有 47.6% 采样
  顶在 ±6500、约每 2.5 ms 反向一次、I5 抖 ±1000 rpm；根因与仿真见 `config.h`）；
  单发角度环按同一份实测重整定：KP=300、限速 4200 rpm（原 85 / 3400 rpm，
  实机单发 275~320 ms → 85~130 ms），松开 S1 后继续追完本次 +40° 目标；
  连发 PLL KP=60、追相位上限 5600 rpm。

### 0.8 发射机构（M3508 摩擦轮）阶跃整定
- 指标：遥控 S2 从 1 拨到 3、目标 0→6000 rpm 的阶跃，"从接到指令到稳态" ≤150 ms。
- `3508.csv` 实测旧参数（KP15/KI0.5/KD0/alpha0.20）为 **210 ms**、过冲 +5.4%
  （峰值 6325 rpm）。同一份数据辨识出的关键量：
  - 被控对象 `a = 4.98·I_实际 + 187 rpm/s`，即 **k_i=4.98 rpm/s per 电流单位**；
  - 满命令 16384 时实际电流只有 ~14.7k@0rpm，并随转速升到 ~10.2k@5000rpm
    （反电动势/电调限流）；6000 rpm 稳态约需 400 单位；
  - **命令→实际转矩电流有 8~9 ms 滞后**（用 I4 拟合加速度残差 9548，
    换成 I5 在 lag=0 拟合残差 6360，说明是电调电流环跟不上命令）。
- 结论：过冲 ≈ 上升期加速度 70000 rpm/s × 速度低通 τ（alpha=0.20 → 5 ms），
  瓶颈是"电调 8 ms + 低通 5 ms"的环路总延时；纯 P 环只能靠降增益换稳定
  （KD=0 时最坏 214 ms，够不到 150 ms），对策是去掉低通滞后 + 加微分超前。
- 现值：KP=18、KI=20、KD=100（`kd·(e[k]-e[k-1])` 每拍差量语义，等效微分时间
  ≈5.6 ms，**不是**每秒导数）、速度低通 alpha=0.50、积分分离 1000 rpm、限幅 16384。
- 仿真（延时 6/8/10 ms × 负载 300/400/600 共 9 工况）最坏：±1% 稳态 136 ms、
  ±0.5% 稳态 141 ms、过冲 +0.21%、静差 ≤15 rpm。
- 代价与回退：KD 会把转速反馈噪声放大成电流纹波；若保持段电流持续 ±2000 以上
  跳动，把 KD 降到 60（最坏 151 ms）。
- 复测入口：`VOFA_LAUNCH_M3508_TUNING_MODE=1`（该页带固件侧时间轴通道，
  不要用上位机行号×5 ms 推时间）。
...
- S1 三档控制 M2006 拨弹（电机轴带 1:36 减速箱）：
  1 保持、2 连发（角度环作锁相环，相位按 40°/50 ms=800°/s 推进）、3 单动（从 1 拨到 3 触发一次 40°输出）。
...
- VOFA 当前走默认综合页：Pitch/Yaw 目标与实际角、M2006 拨盘目标/实际输出角、
  两颗 M3508 实测转速；各轴调参页用 `config.h` 的 `VOFA_*_TUNING_MODE` 开。
...
| S1=2 | M2006 连发：角度环作锁相环，基准 4800 rpm（40°/50 ms ≈20 Hz），相位落后自动补速 |
| S1=3 | M2006 单动：从 S1=1 拨到 3 触发一次 40°输出后保持（实机 85~130 ms 到位）|
| 任意一颗 M3508 离线 | 离线电机停止，在线电机仍按 S2 目标运行 |
| S2=3 | 两颗 M3508 反向同步运行，0→6000 rpm 阶跃约 130~145 ms 进入稳态 |
...
- `Update_launch_para`：传输 S1/S2 三档映射后的发射目标速度、拨弹开关状态和在线状态。
...
| 通道 | 功能 |
|---|---|
| S1 | M2006 拨弹：1 保持，2 连发(锁相环 800°/s≈20 Hz)，3 单动(1→3 触发 40°) |
...
- S1=2 连发（角度环作锁相环，目标相位 800°/s），S1=3 单动步进 40°；
...
UART4 每 5 ms 按绝对周期发送 8 个小端 float，随后发送帧尾 `00 00 80 7F`。四个专用
调参页当前全关，走**默认综合页**：Pitch/Yaw 目标与实际角、M2006 拨盘目标/实际输出角、
两颗 M3508 实测转速。需要调参时把对应宏置 1（`vofa.c` 是 `#elif` 链，只能开一个）：
拨盘单发/连发切 `VOFA_LAUNCH_TUNING_MODE`，摩擦轮 M3508 阶跃（0→6000 rpm/150 ms）
切 `VOFA_LAUNCH_M3508_TUNING_MODE`，Yaw 阶跃切 `VOFA_YAW_TUNING_MODE`，Pitch 调参
切 `VOFA_PITCH_TUNING_MODE`。完整通道映射、命令清单和调试/操作宏见
...
- 修改 `LAUNCH_M2006_ID5_STEP_DEG`（单发 40°）、连发步进周期或 M2006 限速前，
  应确认 M2006 减速比、机构允许速度和供弹安全性。
