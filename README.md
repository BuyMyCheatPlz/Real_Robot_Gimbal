...
### 0.5 Pitch 大负载防超调基线
- 速度 PID 现在按“最终输出限幅减去前馈”得到动态上下限；重力/加速度前馈占用输出余量时，
  积分器能够看到真正的饱和边界，不再发生二次裁剪导致的 windup。
- 默认值改成安全整定基线：轨迹 `1.2 rad/s、8 rad/s²`，速度限幅 `70°/s`，位置
  `Kp=4、Ki=0、Kd=0`，速度 `Ki=0`。先验证内速度环，再逐级提速。
- 预留惯量加速度前馈 `PITCH_ACCEL_FF_VOLTAGE_PER_RAD_S2`，默认 0；只有速度环和重力
  前馈整定完成后才启用。
- 可在线修改 `PITCH_TRAJ_SPEED`、`PITCH_TRAJ_ACCEL`、`PITCH_MAX_SPEED`、
  `PITCH_VEL_FF`、`PITCH_ACCEL_FF`，无需反复编译。
...
- 使用连续 Pitch 编码器角计算正弦重力电压前馈，降低云台负载对 PID 的影响，并避免 IMU 融合角滞后进入前馈。
...
| `VOFA_print` | 10 ms | UART4 DMA 发送 8 通道 JustFloat |
...
- 根据连续 Pitch 编码器角计算正弦重力电压前馈。
- 上电后用首次有效、映射到 `roll_rad` 的 BMI088 姿态角给编码器建立零偏，
  目标设为 `PITCH_GRAVITY_ZERO_RAD`。
