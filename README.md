...
| 通道 | 功能 |
|---|---|
| S1 | M2006 目标转速：1 停止，2 低速，3 为最高速 |
...
UART4 每 10 ms 发送 16 个小端 float，随后发送帧尾 `00 00 80 7F`。通道同时输出
Yaw/Pitch 的控制目标、编码器/速度/命令实际值、撤权原因与 BMI088 的
Roll/Pitch/Yaw 解算角；即使云台轴未被授权也持续发送。完整映射见
